...
def print_death_age(people_dict):
    ...
    for i in people_dict:
        person_key = people_dict[i] 

        name = person_key[0]
        dob = person_key[2]
        dod = person_key[3]

        age_at_death = dod - dob

        print(f"{name}, {dob}, {dod}: {age_at_death}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for i in marriages_dict:
        marriage_key = marriages_dict[i]
        male_info = people_dict[marriage_key[0]]
        female_info = people_dict[marriage_key[1]]
        wed_date_h = marriage_key[2] - male_info[2]
        wed_date_w = marriage_key[2] - female_info[2]
        print(f"{male_info[0]} {wed_date_h} > {marriage_key[2]} < {female_info[0]} {wed_date_w}")
...
def count_marriages(marriages_dict, people_dict):
    ...
    for i in marriages_dict:
        marriage_key = marriages_dict[i]
        male_name = people_dict[marriage_key[0]]
        female_name = people_dict[marriage_key[1]]
        marriage_count.append(male_name[0])
        marriage_count.append(female_name[0])
        
    marriage_count = Counter(marriage_count)
    for i in marriage_count:
        print(f"{i}, {marriage_count[i]}")
    

    

    # print(f"{husband_name} {times_married_h}, {wife_name} {times_married_w}")
