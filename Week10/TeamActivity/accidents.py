...
def main():
    ...
    filename = nameInput("Name of file that contains NHTSA data: ")
    with open(filename, "rt") as text_file:

        # Prompt the user for a percentage.
        perc_reduc = floatinput("Percent reduction of texting while driving [0, 100]: ")
...
def floatinput(msg):
    ...
    while True:
        try:
            perc_reduc = float(input(msg))
            if perc_reduc > 100:
                print(f"Error: {perc_reduc:.1f} is too high. Please enter a different number.")
            elif perc_reduc < 0:
                print(f"Error: {perc_reduc:.1f} is too low. Please enter a different number.")
            else:
                return perc_reduc
